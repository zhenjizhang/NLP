...
const char *out_put = "2015110773.txt"; //输出文件
...
void dic_init(void) //初始化词典
{
	...
	for(int i = 0; i < count_text; i++)
	{
		file >> word_text;
		file >> rate_text;
		rate_text = rate_text / count_text;
		dic.insert(pair<string, double>(word_text, rate_text));
	}
	cout << "dic_init complete!" << "\n";
	file.close();
}
		...
		for(int i = 0; i < word_pool.size(); i++) //遍历整个单词池
		{
			if(-1 == (word_pool.at(i)).best) //没有酸楚左邻词
			{
				...
				for(int j = 0; j < word_pool.size(); j++) //遍历整个单词池
				{
					...
					cout <<"word_pool.size()"<<word_pool.size()<<" j="<<j<<" t5="<<t5<<" t6="<<t6<<" t7="<<t7<<"\n";
					t7++;
				}
				if(0 != best_word_temp.size()) //左右备选项资料完备
				{
					...
					for(int k = 1; k<best_word_temp.size(); k++) //遍历，找到概率最大的
					{
						...
						cout <<"best_word_temp.size()"<<best_word_temp.size()<<" k="<<k<<" t5="<<t5<<"\n";
					}
					//记录左邻词和概率
			...
			cout<<"word_pool.size="<<word_pool.size()<<"t5= "<<t5<<" t6="<<t6<<"\n";
			t6++;
		}
		cout<<"t5="<<t5<<"\n";
		t5++;
