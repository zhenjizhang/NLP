...
string segword(stirng sentance) //最大概率分词，输入为不带“。”的句子，输出为分好的句子
{
	if(sentance.size() < = 2)
	...
	{
		temp_word.clear();
		for(int j = i ; j <= max && j<nax_word + i; j += 2)
		{
			temp_word.append(1, sentance.at(j));
			temp_word.append(1, sentance.at(j + 1));
			//入池
	...
	*/
	...
	while(fail)
	{
		...
		{
			if((word_pool.at(i)).best == -1) //没有算出左邻词
			{
				...
				for(int j = 0; j < word_pool.size(); j++) //遍历整个单词池
				{
					...
					{
						if((word_pool.at(j)).best == -1) //考虑的左邻词本身数据不全
						{
							fail = true; //标记未完成
				...
				if(best_word_temp.size() != 0) //左右备选项资料完备
				{
